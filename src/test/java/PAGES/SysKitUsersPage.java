...
public class SysKitUsersPage extends SysKitUsersPageMappings {
    ...
    public static class DeleteUserPopUp extends SysKitUsersPageMappings.DeleteUserPopUp {
        ...
        public static void verifyPopUpContent(Page page, String sUserFullName) {
            ErrorHandler.errorWrapper("Verify content of the 'Delete User' pop-up on the SysKit Users page", false,
                    success -> {
                ...
                if (!page.isVisible(mContent))
                    ...
                    if (!sActualTitle.equals(sExpectedTitle))
                        lsExceptions.add(
                                "Title of the 'Delete User' pop-up isn't as expected!\nExpected: " + sExpectedTitle
                                        + ", Actual: " + sActualTitle);
                    ...
                    if (!sActualContent.equals(sExpectedContent))
                        lsExceptions.add(
                                "Content of the 'Delete User' pop-up isn't as expected!\nExpected:\n" + sExpectedContent
                                        + "\nActual:\n" + sActualContent);
                }
        ...
        public static void clickButtonDeleteUser(Page page) {
            ErrorHandler.errorWrapper(
                    "Click button 'Delete User' on the 'Delete User' pop-up on the SysKit Users page", success -> {
    ...
    public static class ToastNotification extends SysKitUsersPageMappings.ToastNotification {

        public static void verifyToastNotificationInProgressContent(Page page) {
            ErrorHandler.errorWrapper("Verify content of the Toast notification (in progress) on the SysKit Users "
                    + "page", false, success -> {
                List<String> lsExceptions = new ArrayList<>();
                page.waitForSelector(mToastifyTitle, new Page.WaitForSelectorOptions().setTimeout(10000));
                if (!page.isVisible(mToastifyTitle))
                    lsExceptions.add("Toast notification (in progress) isn't displayed on the SysKit Users page!");
                else {
                    String sActualTitle = page.querySelector(mToastifyTitle).textContent();
                    if (!sActualTitle.equals(sExpectedTitleDeleteUser))
                        lsExceptions.add("Title of the Toast notification (in progress) isn't as expected!\nExpected: "
                                + sExpectedTitleDeleteUser + ", Actual: " + sActualTitle);
                    ...
                    if (!sActualContent.equals(sExpectedMessageDeleteUserProgress))
                        lsExceptions.add(
                                "Content of the Toast notification (in progress) isn't as expected!\nExpected:\n"
        ...
        public static void waitUntilError(Page page) {
            ErrorHandler.errorWrapper(
                    "Wait until the Toast notification (completed) display the error message on the SysKit "
                        ...
                        if (!bSuccess)
                            lsExceptions.add(
                                    "Toast notification (completed) with error message isn't displayed on the SysKit "
                                            + "Users page!");
                        if (lsExceptions.size() > 0)
                            success.exception = new Exception("\n" + String.join("\n", lsExceptions) + "\n");
                        return success;
        ...
        public static void clickLinkToCheckDetails(Page page) {
            ErrorHandler.errorWrapper("Click 'Check details' link on the Toast notification on the SysKit Users page"
                    , success -> {
    ...
    public static void search(Page page, String sValue) {
        ErrorHandler.errorWrapper("Search for '" + sValue + "' on the SysKit Users page", false, success -> {
            ...
            try {
                page.waitForSelector(mLoader, new Page.WaitForSelectorOptions().setTimeout(3000));
            } catch (Exception e) {
            }
    ...
    public static String getUsernameFromTheTable(Page page, int iRow) {
        ...
        ErrorHandler.errorWrapper(
                "Get username from the table row " + (iRow + 1) + " on the SysKit Users page", false, success -> {
                    List<String> lsExceptions = new ArrayList<>();
                    ...
                    if (iRow > lsTableRows.size())
                        ...
                        sUsername[0] = lsTableRows.get(iRow).querySelector(mTableUsernameColumn).innerText();
                    if (lsExceptions.size() > 0)
                        success.exception = new Exception("\n" + String.join("\n", lsExceptions) + "\n");
                    return success;
                });
    ...
    public static String getDisplayNameFromTheTable(Page page, int iRow) {
        ...
        ErrorHandler.errorWrapper(
                "Get Display name from the table row " + (iRow + 1) + " on the SysKit Users page", false, success -> {
                    List<String> lsExceptions = new ArrayList<>();
                    page.waitForSelector(mTableRows);
                    List<ElementHandle> lsTableRows = page.querySelectorAll(mTableRows);
                    if (iRow > lsTableRows.size())
                        ...
                        sDisplayName[0] = lsTableRows.get(iRow).querySelector(mTableDisplayNameColumn).innerText();
                    if (lsExceptions.size() > 0)
                        success.exception = new Exception("\n" + String.join("\n", lsExceptions) + "\n");
                    return success;
                });
    ...
    public static void verifyDisplayNameInTheTable(Page page, int iRow, String sExpectedDisplayName) {
        ErrorHandler.errorWrapper(
                "Verify Display name in the table row " + (iRow + 1) + " on the SysKit Users page is '"
                        + sExpectedDisplayName + "'", false, success -> {
                    List<String> lsExceptions = new ArrayList<>();
                    page.waitForSelector(mTableRows);
                    List<ElementHandle> lsTableRows = page.querySelectorAll(mTableRows);
                    if (iRow > lsTableRows.size())
                        ...
                        String sActualDisplayName =
                                lsTableRows.get(iRow).querySelector(mTableDisplayNameColumn).innerText();
                        if (!sActualDisplayName.equals(sExpectedDisplayName))
                            lsExceptions.add("Value in the table row " + (iRow + 1)
                                    + " isn't as expected on the SysKit Users page!\nExpected: " + sExpectedDisplayName
                                    + "\nActual: " + sActualDisplayName);
                    }
                    if (lsExceptions.size() > 0)
                        success.exception = new Exception("\n" + String.join("\n", lsExceptions) + "\n");
                    return success;
    ...
    public static void verifyNumberOfTableRows(Page page, int iExpectedNumberOfRows) {
        ErrorHandler.errorWrapper(
                "Verify number of rows in the table on the SysKit Users page is '" + iExpectedNumberOfRows
                        + "'", false, success -> {
                    List<String> lsExceptions = new ArrayList<>();
                    page.waitForSelector(mTableRows);
                    ...
                    if (iActualNumberOfRows != iExpectedNumberOfRows)
                        lsExceptions.add(
                                "Number of rows in the table on the SysKit Users page isn't as expected!\nExpected: "
